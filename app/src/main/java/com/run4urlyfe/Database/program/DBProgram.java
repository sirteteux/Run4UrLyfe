...
public class DBProgram extends DBBase {
    ...
    public long add(Program m) {
        ...
        value.put(DBProgram.NAME, m.getName());
        value.put(DBProgram.DESCRIPTION, m.getDescription());

        long new_id = db.insert(DBProgram.TABLE_NAME, null, value);
    ...
    public Program get(long id) {
        ...
        if (mCursor != null && mCursor.getCount() > 0) {
            ...
            Program value = new Program(mCursor.getLong(mCursor.getColumnIndex(DBProgram.KEY)),
                    mCursor.getString(mCursor.getColumnIndex(DBProgram.NAME)),
                    mCursor.getString(mCursor.getColumnIndex(DBProgram.DESCRIPTION))
            );
            mCursor.close();
            close();
