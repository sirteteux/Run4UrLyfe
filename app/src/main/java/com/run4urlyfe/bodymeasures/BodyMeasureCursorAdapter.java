...
public class BodyMeasureCursorAdapter extends CursorAdapter {
    ...
    public void bindView(View view, Context context, Cursor cursor) {
        ...
        Date date = DateConverter.DBDateStrToDate(cursor.getString(cursor.getColumnIndex(DBBodyMeasure.DATE)));
        String dateStr = DateConverter.dateToLocalDateStr(date, mContext);
        t1.setText(dateStr);

        float measure = cursor.getFloat(cursor.getColumnIndex(DBBodyMeasure.MEASURE));
        Unit unit = Unit.fromInteger(cursor.getInt(cursor.getColumnIndex(DBBodyMeasure.UNIT)));
        ...
        editImg.setTag(cursor.getLong(cursor.getColumnIndex(DBBodyMeasure.KEY)));
        ...
        deletImg.setTag(cursor.getLong(cursor.getColumnIndex(DBBodyMeasure.KEY)));
