...
public class SourceGraphFragment extends Fragment {
    MainActivity mActivity = null;
    ArrayAdapter<String> mAdapterMachine = null;
    //Profile mProfil = null;
    ...
    private DBSource mDbSource = null;
    private DBCardio mDbCardio = null;
    private DBStatic mDbStatic = null;
    private DBMachine mDbMachine = null;
     ...
     */
    public static SourceGraphFragment newInstance(String name, int id) {
        SourceGraphFragment f = new SourceGraphFragment();
    ...
    public View onCreateView(LayoutInflater inflater, ViewGroup container,
        ...
        if (mDbSource == null) mDbSource = new DBSource(getContext());
        if (mDbCardio == null) mDbCardio = new DBCardio(getContext());
        if (mDbStatic == null) mDbStatic = new DBStatic(getContext());
        if (mDbMachine == null) mDbMachine = new DBMachine(getContext());
    ...
    public void onStart() {
        ...
        if (getProfile() != null) {
            mMachinesArray = new ArrayList<>(0); //Data are refreshed on show //mDbSource.getAllMachinesStrList(getProfil());
    ...
    private void drawGraph() {
        ...
        DBMachine mDbExercise = new DBMachine(mActivity);
        ...
        if (m.getType() == ExerciseType.STRENGTH) {
            ...
            if (lFunction.equals(mActivity.getResources().getString(R.string.sumDistance))) {
                lDAOFunction = com.run4urlyfe.Database.record.DBCardio.DISTANCE_FCT;
                ...
                lDAOFunction = com.run4urlyfe.Database.record.DBCardio.DURATION_FCT;
                desc = lMachine + "/" + lFunction + "(min)";
            } else if (lFunction.equals(mActivity.getResources().getString(R.string.speed))) {
                lDAOFunction = com.run4urlyfe.Database.record.DBCardio.SPEED_FCT;
            ...
            for (int i = 0; i < valueList.size(); i++) {
                Entry value = null;
                if (lDAOFunction == com.run4urlyfe.Database.record.DBCardio.DURATION_FCT) {
                    value = new Entry((float) valueList.get(i).getX(), (float) DateConverter.nbMinutes(valueList.get(i).getY()));
                } else if (lDAOFunction == com.run4urlyfe.Database.record.DBCardio.SPEED_FCT) { // Km/h
        ...
        if (m.getType() == ExerciseType.ISOMETRIC) {
            ...
            if (lDAOFunction == com.run4urlyfe.Database.record.DBStatic.MAX_FCT) {
                WeightUnit defaultUnit = SettingsFragment.getDefaultWeightUnit(getMainActivity());
            ...
            } else if (lDAOFunction == com.run4urlyfe.Database.record.DBStatic.NBSERIE_FCT || lDAOFunction == com.run4urlyfe.Database.record.DBStatic.MAX_LENGTH) {
                for (int i = 0; i < valueList.size(); i++) {
                    Entry value = new Entry((float) valueList.get(i).getX(), (float) valueList.get(i).getY());
                    yVals.add(value);
                }
        ...
        if (mFragmentView != null) {
            if (getProfile() != null) {
                if (mAdapterMachine == null) {
                    mMachinesArray = mDbSource.getAllMachinesStrList();
                    ...
                    if (mMachinesArray == null)
                        ...
                        mMachinesArray.addAll(mDbSource.getAllMachinesStrList());
                        mAdapterMachine.notifyDataSetChanged();
                        mDbSource.closeCursor();
                    }
                }

                int position = mAdapterMachine.getPosition(this.getSourcesMachine());
                if (position != -1) {
    ...
    private String getSourcesMachine() {
